...
class ElectrumWindow(QMainWindow):
    ...
    def __init__(self, config, network, gui_object):
        ...
        self.setWindowIcon(QIcon(":icons/electrum-prux.png"))
        self.init_menubar()
    ...
    def load_wallet(self, wallet):
        ...
        title = 'Electrum-PRUX %s  -  %s' % (self.wallet.electrum_version, self.wallet.basename())
    ...
    def init_menubar(self):
        ...
        help_menu.addAction(_("&Official website"), lambda: webbrowser.open("http://electrum-prux.org"))
    ...
    def show_about(self):
        QMessageBox.about(self, "Electrum-PRUX",
            _("Version")+" %s" % (self.wallet.electrum_version) + "\n\n" + _("Electrum's focus is speed, with low resource usage and simplifying Prux. You do not need to perform regular backups, because your wallet can be recovered from a secret phrase that you can memorize or write on paper. Startup times are instant because it operates in conjunction with high-performance servers that handle the most complicated parts of the Prux system."))

    def show_report_bug(self):
        QMessageBox.information(self, "Electrum-PRUX - " + _("Reporting Bugs"),
            _("Please report any bugs as issues on github:")+" <a href=\"https://github.com/pooler/electrum-prux/issues\">https://github.com/pooler/electrum-prux/issues</a>")
    ...
    def notify(self, message):
        if self.tray:
            self.tray.showMessage("Electrum-PRUX", message, QSystemTrayIcon.Information, 20000)
    ...
    def create_send_tab(self):
        ...
        msg = _('Recipient of the funds.') + '\n\n'\
              + _('You may enter a Prux address, a label from your list of contacts (a list of completions will be proposed), or an alias (email-like address that forwards to a Prux address)')
        ...
        msg = _('Prux transactions are in general not free. A transaction fee is paid by the sender of the funds.') + '\n\n'\
    ...
    def read_send_tab(self):
        ...
        for _type, addr, amount in outputs:
            if addr is None:
                QMessageBox.warning(self, _('Error'), _('Prux Address is None'), _('OK'))
                return
            if _type == 'address' and not bitcoin.is_address(addr):
                QMessageBox.warning(self, _('Error'), _('Invalid Prux Address'), _('OK'))
                return
            if amount is None:
    ...
    def export_privkeys_dialog(self, password):
        ...
        defaultname = 'electrum-prux-private-keys.csv'
    ...
    def export_history_dialog(self):
        ...
        vbox = QVBoxLayout(d)
        defaultname = os.path.expanduser('~/electrum-prux-history.csv')
